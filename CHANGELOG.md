...
not whether Python permits a consumer to write these, but whether any Python consumer *does*. Across
...
- **The re-peering path cleared `syncBackoff` and `nextSyncAttempt` cross-object.** Those are the
  sync machine's own—`sync()` computes `nextSyncAttempt = now + syncBackoff` under `peerLock`—so
  clearing them from the announce thread could tear that deadline.
...
- **An existing node with more peers than `maxPeers` rotates down to the bound on first run.**
  There was no bound before, so a long-running node may hold more than 20 peers; rotation drops the
  worst-performing ones over successive passes. This is the intended behaviour and not data loss—the
...
- **Paper messages carried the message in cleartext** (`bugs/026`). Python encrypts the payload
  ...
  it—on a feature whose entire purpose is to cross a channel the sender does not control—and
  no Python client could ingest one. `pack()` now builds the paper form and throws
  `paperMDUExceeded` past `PAPER_MDU` rather than handing back a URI that silently does not fit
  in a QR code.
...
- **`.delivered` now dwells in `.sending` until the recipient proves receipt, and a retry after
  a timeout is visible.** This reads as a regression to anyone who has not been told: the
...
- **Propagation sync progress was only readable after the sync finished.**
  ...
  that is, once the transfer had completed—precisely when a progress display no
