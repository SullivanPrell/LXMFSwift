...
public final class LXMessage {
  ...
  public func pack() throws {
    ...
    self.propagationPacked = MsgPack.encode(
      .array([
        .double(ts),
    ...
    selectMethod(
      contentSize: corePayload.count - LXMessage.timestampSize - LXMessage.structOverhead)
  }
  ...
  public static func unpack(_ data: Data) throws -> LXMessage {
    ...
    var cursor = 0
    let destHash = data[cursor..<cursor + hlen]
    cursor += hlen
    let srcHash = data[cursor..<cursor + hlen]
    cursor += hlen
    let sig = data[cursor..<cursor + slen]
    cursor += slen
  ...
  public func validateStamp(targetCost: Int, tickets: [Data]? = nil) -> Bool {
    ...
    if let tickets {
      for ticket in tickets where s == Hashes.truncatedHash(ticket + msgHash) {
        stampValid = true
...
extension LXMessage {
  ...
  public static let uriSchema = "lxm"
  ...
  public func setTitleFromString(_ string: String) { title = Data(string.utf8) }

  /// Mirrors Python's `LXMessage.set_title_from_bytes(title_bytes)`.
  public func setTitleFromBytes(_ bytes: Data) { title = bytes }

  /// Mirrors Python's `LXMessage.set_content_from_string(content_string)`.
  public func setContentFromString(_ string: String) { content = Data(string.utf8) }

  /// Mirrors Python's `LXMessage.set_content_from_bytes(content_bytes)`.
  public func setContentFromBytes(_ bytes: Data) { content = bytes }

  // MARK: Fields

  /// Mirrors Python's `LXMessage.set_fields(fields)`.
  public func setFields(_ newFields: [Int: Any]?) { fields = newFields ?? [:] }

  /// Mirrors Python's `LXMessage.get_fields()`.
  public func getFields() -> [Int: Any]? { fields }
  ...
  public func asURI() throws -> String {
  ...
  public func asQR() throws -> CIImage {
    guard desiredMethod == .paper else {
      throw LXMessageError.notPaperMethod
    }
    if packed == nil { try pack() }
  ...
  public static func paperData(fromURI uri: String) throws -> Data {
    ...
    guard let data = Data(base64Encoded: paddedStr),
      data.count > LXMessage.destinationLength
    else {
      throw LXMessageError.invalidURI
    }
    return data
  ...
  public static func fromURI(_ uri: String, destination: Destination) throws -> LXMessage {
  ...
  public func getPropagationStamp(targetCost: Int) -> Data? {
    ...
    guard
      let stamp = LXStamper.generateStamp(
        messageID: tid,
  ...
  public func attachPropagationStamp(cost: Int) {
    ...
    guard
      let stamp = LXStamper.generateStamp(
        messageID: transientID, stampCost: cost,
        expandRounds: LXStamper.pnExpandRounds)
    else { return }

    propagationStamp = stamp
    let newLxmfData = lxmfDataNoStamp + stamp
    propagationPacked = MsgPack.encode(
      .array([
        .double(ts),
  ...
  private static func tryUnpackContainer(_ data: Data) -> LXMessage? {
    ...
    // Find "lxmf_bytes"
    guard
      let lxmfBytesVal = pairs.first(where: {
        if case .string(let s) = $0.0 { return s == "lxmf_bytes" }
        return false
      })?.1, case .bytes(let lxmfBytes) = lxmfBytesVal
    else { return nil }
    ...
    if let stateVal = pairs.first(where: {
      if case .string(let s) = $0.0 { return s == "state" }
      return false
    })?.1 {
      if case .uint(let v) = stateVal,
        let s = LXMessage.State(rawValue: UInt8(v))
      {
        msg.state = s
    ...
    if let methodVal = pairs.first(where: {
      if case .string(let s) = $0.0 { return s == "method" }
      return false
    })?.1 {
      if case .uint(let v) = methodVal,
        let m = LXMessage.Method(rawValue: UInt8(v))
      {
        msg.method = m
    ...
    if let teVal = pairs.first(where: {
      if case .string(let s) = $0.0 { return s == "transport_encrypted" }
      return false
    ...
    if let encVal = pairs.first(where: {
      if case .string(let s) = $0.0 { return s == "transport_encryption" }
      return false
