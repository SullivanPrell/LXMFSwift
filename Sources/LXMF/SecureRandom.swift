...
/// call site: an unchecked `SecRandomCopyBytes` reports nothing on failure and leaves the
/// buffer zero-filled.
///
/// ReticulumSwift declares an identical `SecureRandom`, but this package builds against that
/// one's published release, and the type is not in one yet. Delete this file and drop the
/// three call sites onto the dependency's copy once it ships.
enum SecureRandom {
