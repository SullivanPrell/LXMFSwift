...
public final class LXMRouter {
  ...
  var pendingOutbound: [LXMessage] {
    lock.lock()
    defer { lock.unlock() }
    return unsafePendingOutbound
  }
  private var unsafePendingOutbound: [LXMessage] = []
  ...
  public var propagationEntries: [Data: PropagationEntry] {
    lock.lock()
    defer { lock.unlock() }
    return unsafePropagationEntries
  }
  ...
  public var peerDistributionQueue: [(transientID: Data, fromPeer: LXMPeer?)] {
    lock.lock()
    defer { lock.unlock() }
    return unsafePeerDistributionQueue
  }
  private var unsafePeerDistributionQueue: [(transientID: Data, fromPeer: LXMPeer?)] = []

  /// Number of messages received from unpeered clients.
  public var clientPropagationMessagesReceived: Int {
    lock.lock()
    defer { lock.unlock() }
    return unsafeClientPropagationMessagesReceived
  }
  private var unsafeClientPropagationMessagesReceived: Int = 0

  /// Number of messages served to clients.
  public var clientPropagationMessagesServed: Int {
    lock.lock()
    defer { lock.unlock() }
    return unsafeClientPropagationMessagesServed
  }
  private var unsafeClientPropagationMessagesServed: Int = 0

  /// Number of propagation messages from unpeered nodes.
  public var unpeeredPropagationIncoming: Int {
    lock.lock()
    defer { lock.unlock() }
    return unsafeUnpeeredPropagationIncoming
  }
  private var unsafeUnpeeredPropagationIncoming: Int = 0

  /// Bytes received from unpeered propagation sources.
  public var unpeeredPropagationRxBytes: Int {
    lock.lock()
    defer { lock.unlock() }
    return unsafeUnpeeredPropagationRxBytes
  }
  private var unsafeUnpeeredPropagationRxBytes: Int = 0
  ...
  public func getAnnounceAppData(destinationHash: Data) -> Data? {
    ...
    let supportedFunctionality: MsgPack.Value = .array([.uint(UInt64(sfCompression))])
  ...
  @discardableResult
  public func announcePropagationNode(attachedInterface: (any Interface)? = nil) throws
    -> PacketReceipt?
  {
  ...
  public func getOutboundTicket(destinationHash: Data) -> Data? {
    ...
    guard let entry = outboundTickets[destinationHash],
      entry.expiry > Date().timeIntervalSince1970
    else { return nil }
    return entry.ticket
  }
    ...
    -> (expiry: TimeInterval, ticket: Data)?
  {
    ...
    if let existing = inboundTickets[destinationHash] {
    ...
    inboundTickets[destinationHash, default: [:]][newTicket] = newExpiry
    lock.unlock()
  ...
  public func getInboundTickets(destinationHash: Data) -> [Data]? {
    ...
    defer { lock.unlock() }
    let now = Date().timeIntervalSince1970
    guard let tickets = inboundTickets[destinationHash] else { return nil }
  ...
  public func cleanAvailableTickets() {
    ...
    for (destHash, tickets) in inboundTickets {
      inboundTickets[destHash] = tickets.filter { $0.value + LXMessage.ticketGrace > now }
    }
  }
  ...
  public func isEnforcingStamps() -> Bool { stampsEnforced }
  ...
  func inboundRegistryKeys() -> [Data] {
    incomingDeliveryResourceLock.lock()
    defer { incomingDeliveryResourceLock.unlock() }
  ...
  public func cancelOutbound(messageID: Data) {
    lock.lock()
    if let idx = unsafePendingOutbound.firstIndex(where: { $0.messageID == messageID }) {
      unsafePendingOutbound[idx].state = .cancelled
    }
    unsafePendingOutbound.removeAll { $0.messageID == messageID && $0.state == .cancelled }
    lock.unlock()
  }
  ...
  public func getOutboundProgress(lxmHash: Data) -> Double? {
    lock.lock()
    defer { lock.unlock() }
    return unsafePendingOutbound.first { $0.hash == lxmHash }?.progress
  }
  ...
  public func ingestLXMURI(_ uri: String) throws -> Bool {
    ...
    lock.lock()
    let destination = deliveryDestinations[destHash]
    lock.unlock()
    ...
    if message.packed == nil {
      ...
      if message.desiredMethod == .propagated {
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
          ...
          guard let self else { return }
          self.lock.lock()
          self.unsafePendingOutbound.append(message)
          self.lock.unlock()
  ...
  public func processOutbound() {
    lock.lock()
    let snapshot = unsafePendingOutbound
    lock.unlock()
  ...
  func handleMessageGetResponse(_ data: Data, receipt: RequestReceipt) {
    ...
    for value in msgValues {
      ...
      lock.lock()
      let dest = deliveryDestinations[destHash]
      lock.unlock()
      guard let dest,
        let plaintext = try? dest.decrypt(encryptedPayload)
      else { continue }

      lock.lock()
      locallyDeliveredTransientIDs[transientID] = Date().timeIntervalSince1970
      lock.unlock()
  ...
  public func deliverInboundResource(_ data: Data, noStampEnforcement: Bool = false) -> Bool {
    ...
    guard let srcIdentity = transport.recall(identity: msg.sourceHash) else {
  ...
  public func injectDirectLink(_ link: Link, for destinationHash: Data) {
    lock.lock()
    directLinks[destinationHash] = link
    lock.unlock()
  }
  ...
  func finalizeInboundDelivery(_ msg: LXMessage, noStampEnforcement: Bool = false) -> Bool {
    ...
    if let tid = msg.hash {
      ...
      lock.lock()
      locallyDeliveredTransientIDs[tid] = Date().timeIntervalSince1970
      lock.unlock()
  ...
  private func removePending(_ msg: LXMessage) {
    lock.lock()
    unsafePendingOutbound.removeAll { $0 === msg }
    lock.unlock()
  }
  ...
  public func testInjectPendingOutbound(_ message: LXMessage) {
    lock.lock()
    defer { lock.unlock() }
    if !unsafePendingOutbound.contains(where: { $0 === message }) {
      unsafePendingOutbound.append(message)
    }
  }
  ...
  internal func handleAnnounceForDestination(_ destinationHash: Data) {
    lock.lock()
    let matches = unsafePendingOutbound.filter { $0.destinationHash == destinationHash }
  ...
  public func enablePropagation(storagePath path: String) throws {
    ...
    lock.lock()
    unsafePropagationEntries.removeAll()
    lock.unlock()
      ...
      case .array(let peerList) = (try? MsgPack.decode(peersData)) ?? .nil
    {
      for item in peerList {
        ...
        {
          lock.lock()
          unsafePeers[peer.destinationHash] = peer
          lock.unlock()
        }
      }
    }
      ...
      case .map(let statsPairs) = (try? MsgPack.decode(statsData)) ?? .nil
    {
      ...
      if let v = statsInt("client_propagation_messages_received") {
        unsafeClientPropagationMessagesReceived = v
      }
      if let v = statsInt("client_propagation_messages_served") {
        unsafeClientPropagationMessagesServed = v
      }
      if let v = statsInt("unpeered_propagation_incoming") {
        unsafeUnpeeredPropagationIncoming = v
      }
      if let v = statsInt("unpeered_propagation_rx_bytes") {
        unsafeUnpeeredPropagationRxBytes = v
      }
    }
    ...
    propagationDestination?.registerNativeRequestHandler(
      ...
      return self.handleMessageGetRequest(
        data: requestData,
    ...
    propagationDestination?.onLinkEstablished = { [weak self] link in
      ...
      link.onClosed = { [weak self] l in
        ...
        guard let self else { return }
        self.lock.lock()
        self.unsafeValidatedPeerLinks.removeValue(forKey: ObjectIdentifier(l))
        self.unsafeActivePropagationLinks.removeValue(forKey: ObjectIdentifier(l))
        self.lock.unlock()
      }
    }
  }
  ...
  func handleInboundPropagationPacket(_ data: Data, on link: Link) {
    guard case .array(let outer) = (try? MsgPack.decode(data)) ?? .nil,
      outer.count >= 2,
      case .array(let messages) = outer[1]
    else { return }
    ...
    for entry in validated {
      lock.lock()
      unsafeClientPropagationMessagesReceived += 1
      lock.unlock()
      _ = ingestPropagatedLXM(
    ...
    if validated.count == messages.count {
      ...
      if let reject = try? MsgPack.encode(
        .array([.uint(UInt64(LXMPeerError.invalidStamp.rawValue))]))
      {
  ...
  public func handleInboundPropagationResource(_ data: Data, on link: Link?) {
    ...
    lock.lock()
    let senderPeer = sender.flatMap { unsafePeers[$0] }
    lock.unlock()
    ...
    for entry in validated {
      ...
      if let senderPeer {
        ...
        lock.lock()
        if sender != nil {
          unsafeUnpeeredPropagationIncoming += 1
          unsafeUnpeeredPropagationRxBytes += entry.lxmfData.count
        } else {
          unsafeClientPropagationMessagesReceived += 1
        }
        lock.unlock()
    ...
    if invalidCount > 0, let sender {
      lock.lock()
      unsafeThrottledPeers[sender] = Date().timeIntervalSince1970 + LXMRouter.pnStampThrottle
      lock.unlock()
      try? link?.teardown()
    }
  }
  ...
  func remotePropagationHash(of link: Link) -> Data? {
    ...
    return try? Destination(
      identity: remoteIdentity, direction: .out, kind: .single,
      appName: appName, aspects: ["propagation"]
    ).hash
  }
  ...
  @discardableResult
  public func addToMessageStore(
    lxmfData: Data, transientID: Data,
    stampValue: Int, stamp: Data
  ) -> PropagationEntry? {
    ...
    if locallyProcessedTransientIDs[transientID] != nil {
      lock.unlock()
      return nil
    }
    lock.unlock()
  ...
  public func removeFromMessageStore(transientID: Data) {
    ...
    lock.lock()
    guard let entry = unsafePropagationEntries.removeValue(forKey: transientID) else {
      lock.unlock()
      return
    }
  ...
  public func cleanMessageStore() {
    ...
    lock.lock()
    var currentSize = unsafePropagationEntries.values.reduce(0) { $0 + $1.msgSize }
    guard currentSize > limit else {
      lock.unlock()
      return
    }
    let sorted = unsafePropagationEntries.sorted { $0.value.received < $1.value.received }
    lock.unlock()
  ...
  func peerEntryExists(_ transientID: Data) -> Bool {
    lock.lock()
    defer { lock.unlock() }
    return unsafePropagationEntries[transientID] != nil
  }
  func peerEntry(_ transientID: Data) -> PropagationEntry? {
    lock.lock()
    defer { lock.unlock() }
    return unsafePropagationEntries[transientID]
  }
  func peerHandledTransientIDs(for destinationHash: Data) -> [Data] {
    lock.lock()
    defer { lock.unlock() }
    return unsafePropagationEntries.compactMap {
      $0.value.handledPeers.contains(destinationHash) ? $0.key : nil
    }
  }
  func peerUnhandledTransientIDs(for destinationHash: Data) -> [Data] {
    ...
    defer { lock.unlock() }
    return unsafePropagationEntries.compactMap {
      $0.value.unhandledPeers.contains(destinationHash) ? $0.key : nil
    }
  }
  ...
  func peerAddHandled(_ transientID: Data, destinationHash: Data) -> Bool {
    lock.lock()
    defer { lock.unlock() }
    guard let entry = unsafePropagationEntries[transientID] else { return false }
    guard !entry.handledPeers.contains(destinationHash) else { return false }
    unsafePropagationEntries[transientID]?.handledPeers.append(destinationHash)
    return true
  }
  @discardableResult
  ...
  func peerRemoveHandled(_ transientID: Data, destinationHash: Data) -> Bool {
    lock.lock()
    defer { lock.unlock() }
    guard unsafePropagationEntries[transientID] != nil else { return false }
    unsafePropagationEntries[transientID]?.handledPeers.removeAll { $0 == destinationHash }
    return true
  }
  @discardableResult
  func peerRemoveUnhandled(_ transientID: Data, destinationHash: Data) -> Bool {
    ...
    defer { lock.unlock() }
    guard unsafePropagationEntries[transientID] != nil else { return false }
    unsafePropagationEntries[transientID]?.unhandledPeers.removeAll { $0 == destinationHash }
    return true
  }
  ...
  func seedThrottledPeer(_ destinationHash: Data, until: TimeInterval?) {
    lock.lock()
    defer { lock.unlock() }
    unsafeThrottledPeers[destinationHash] = until
  }

  /// Set any of the four propagation counters; `nil` leaves one unchanged.
  func seedPropagationCounters(
    clientReceived: Int? = nil,
    ...
    lock.lock()
    defer { lock.unlock() }
    if let v = clientReceived { unsafeClientPropagationMessagesReceived = v }
    if let v = clientServed { unsafeClientPropagationMessagesServed = v }
    if let v = unpeeredIncoming { unsafeUnpeeredPropagationIncoming = v }
  ...
  func makePeerSyncContext(for peer: LXMPeer) -> PeerSyncContext? {
    ...
    guard
      let destination = try? Destination(
        identity: peerIdentity, direction: .out,
  ...
  public func peer(
    ...
    lock.lock()
    let existing = unsafePeers[destinationHash]
    let tableIsFull = unsafePeers.count >= unsafeMaxPeers
    lock.unlock()
    ...
    if let existing {
      ...
      apply(
        announcedTimestamp: timestamp, transferLimit: transferLimit,
  ...
  private func apply(
    ...
    peer.adoptAnnouncedTerms(
      announcedTimestamp: announcedTimestamp,
  ...
  func considerAutopeering(with propagationHash: Data) {
    ...
    lock.lock()
    let alreadyPeered = unsafePeers[propagationHash] != nil
    lock.unlock()
  ...
  func handlePropagationNodeAnnounce(
    destinationHash: Data,
    appData: Data?,
    isPathResponse: Bool
  ) {
    ...
    lock.lock()
    let isStatic = unsafeStaticPeers.contains(destinationHash)
  ...
  func addPeer(
    ...
    lock.lock()
    if let existing = unsafePeers[destinationHash] {
      lock.unlock()
      return existing
    }
    let peer = LXMPeer(
      router: self, destinationHash: destinationHash,
  ...
  public struct Job {
    ...
    init(
      _ name: String, every interval: Int, propagationNodeOnly: Bool = false,
  ...
  public static let jobSchedule: [Job] = [
    ...
    .init(
      "processDeferredStamps", every: jobStampsInterval,
    ...
    .init(
      "savePeers", every: jobSaveInterval, propagationNodeOnly: true,
  ...
  public func jobs() -> [String] {
    ...
    var ran: [String] = []
    for job in LXMRouter.jobSchedule where tick % job.interval == 0 {
  ...
  public func cleanLinks(
    peerSyncMaxInactivity: TimeInterval =
    ...
    let propagation = unsafeActivePropagationLinks
    let peerList = Array(unsafePeers.values)
    lock.unlock()
    ...
    where link.noDataFor() > LXMRouter.propagationLinkMaxInactivity {
      lock.lock()
      unsafeActivePropagationLinks.removeValue(forKey: key)
      lock.unlock()
      try? link.teardown()
    }
  ...
  public func flushQueues() {
    lock.lock()
    let hasPeers = !unsafePeers.isEmpty
    lock.unlock()
  ...
  public func cleanThrottledPeers(now: TimeInterval = Date().timeIntervalSince1970) {
    lock.lock()
    defer { lock.unlock() }
    for (destinationHash, deadline) in unsafeThrottledPeers where now > deadline {
      unsafeThrottledPeers.removeValue(forKey: destinationHash)
    }
  }
  ...
  public func rotatePeers() {
    lock.lock()
    let all = Array(unsafePeers.values)
    let staticHashes = unsafeStaticPeers
    let bound = unsafeMaxPeers
    lock.unlock()
    ...
    let headroom = max(
      1,
      Int(
        (Double(bound) * Double(LXMRouter.rotationHeadroomPct) / 100.0)
          .rounded(.down)))
  ...
  public func removePeer(destinationHash: Data) {
    ...
    for tid in Array(unsafePropagationEntries.keys) {
      ...
      unsafePropagationEntries[tid]?.unhandledPeers.removeAll { $0 == peer.destinationHash }
    }
  }
  ...
  public func enqueueForPeerDistribution(transientID: Data, fromPeer: LXMPeer? = nil) {
    lock.lock()
    defer { lock.unlock() }
    guard !unsafePeerDistributionQueue.contains(where: { $0.transientID == transientID }) else {
      return
    }
  ...
  public func flushPeerDistributionQueue() {
    ...
    lock.lock()
    guard !unsafePeerDistributionQueue.isEmpty else {
      lock.unlock()
      return
    }
    let batch = unsafePeerDistributionQueue
    unsafePeerDistributionQueue.removeAll()
    let peerList = Array(unsafePeers.values)
    lock.unlock()
  ...
  func handleOfferRequest(
    data: MsgPack.Value,
    remoteIdentityHash: Data?,
    propagationHash: Data?,
    linkID: ObjectIdentifier
  ) -> MsgPack.Value {
    ...
    if let propagationHash {
      let now = Date().timeIntervalSince1970
      lock.lock()
      let deadline = unsafeThrottledPeers[propagationHash]
      if let deadline, deadline <= now { unsafeThrottledPeers.removeValue(forKey: propagationHash) }
      lock.unlock()
    ...
    guard case .bytes(let keyBytes) = dataArr[0],
      case .array(let idsArr) = dataArr[1]
    else {
      return .int(Int64(LXMPeerError.invalidData.rawValue))
    }
    ...
    if peeringCost > 0 {
      ...
      let peeringID = LXStamper.peeringID(
        receiverIdentityHash: myHash,
        senderIdentityHash: remoteHash)
      guard
        LXStamper.validatePeeringKey(
  ...
  public func handleMessageGetRequest(
    data: MsgPack.Value,
    remoteDeliveryHash: Data?
  ) -> MsgPack.Value {
    ...
    if wantList == nil && haveList == nil {
      lock.lock()
      let available = unsafePropagationEntries.compactMap { (tid, entry) -> (Data, Int)? in
    ...
    if let have = haveList {
      lock.lock()
      let toPurge = have.filter { unsafePropagationEntries[$0]?.destinationHash == destHash }
      lock.unlock()
    ...
    if let want = wantList {
      ...
      let wantedPaths: [String] = want.compactMap { tid in
        guard let entry = unsafePropagationEntries[tid], entry.destinationHash == destHash else {
          return nil
        }
      ...
      for filePath in wantedPaths {
        ...
        let lxmfBytes =
          data.count > LXStamper.stampSize
    ...
    lock.lock()
    unsafeClientPropagationMessagesServed += responseMessages.count
    lock.unlock()
  ...
  @discardableResult
  public func ingestPropagatedLXM(
    lxmfData: Data, stampValue: Int, stamp: Data,
    fromPeer: LXMPeer? = nil
  ) -> PropagationEntry? {
    let transientID = Hashes.fullHash(lxmfData)
    lock.lock()
    let isDup = unsafePropagationEntries[transientID] != nil
    lock.unlock()
    guard !isDup else { return nil }  // duplicate (addToMessageStore re-checks under lock)

    let entry = addToMessageStore(
      lxmfData: lxmfData, transientID: transientID,
  ...
  public func savePeers() {
    ...
    lock.lock()
    let peerObjects = Array(self.unsafePeers.values)
    lock.unlock()
  ...
  public func saveNodeStats() {
    guard let sp = storagePath else { return }
    lock.lock()
    let rcv = unsafeClientPropagationMessagesReceived
    let srv = unsafeClientPropagationMessagesServed
    let unpIn = unsafeUnpeeredPropagationIncoming
    let unpRx = unsafeUnpeeredPropagationRxBytes
    lock.unlock()
  ...
  private func loadLocallyDeliveredTransientIDs() {
    ...
    lock.lock()
    locallyDeliveredTransientIDs = loaded
    lock.unlock()
  ...
  public func saveLocallyProcessedTransientIDs() {
    guard let sp = storagePath else { return }
    lock.lock()
    let snapshot = locallyProcessedTransientIDs
    lock.unlock()
    let value = MsgPack.Value.map(
      snapshot.map { (MsgPack.Value.bytes($0.key), MsgPack.Value.double($0.value)) })
    try? MsgPack.encode(value).write(
  ...
  private func loadLocallyProcessedTransientIDs() {
    guard let sp = storagePath,
      ...
      case .map(let pairs) = (try? MsgPack.decode(data)) ?? .nil
    else { return }
    var loaded: [Data: TimeInterval] = [:]
    for (k, v) in pairs {
      guard case .bytes(let id) = k else { continue }
    ...
    lock.lock()
    locallyProcessedTransientIDs = loaded
    lock.unlock()
  }
  ...
  public func saveOutboundStampCosts() {
    guard let sp = storagePath else { return }
    lock.lock()
    let snapshot = outboundStampCosts
    lock.unlock()
  ...
  public func saveAvailableTickets() {
    guard let sp = storagePath else { return }
    lock.lock()
    let ob = outboundTickets
    let ib = inboundTickets
    let ld = lastDeliveries
    lock.unlock()
    // outbound: { dest: [expiry, ticket] }
    let obValue = MsgPack.Value.map(
      ob.map { (dest, entry) in
        (
          MsgPack.Value.bytes(dest),
          MsgPack.Value.array([.double(entry.expiry), .bytes(entry.ticket)])
        )
      })
    // inbound: { dest: { ticket: expiry } }
    let ibValue = MsgPack.Value.map(
      ib.map { (dest, tickets) in
        (
          MsgPack.Value.bytes(dest),
          MsgPack.Value.map(
            tickets.map { (t, e) in (MsgPack.Value.bytes(t), MsgPack.Value.double(e)) })
        )
      })
    // last_deliveries: { dest: timestamp }
    let ldValue = MsgPack.Value.map(
      ld.map { (dest, ts) in
  ...
  private func loadAvailableTickets() {
    guard let sp = storagePath,
      ...
      case .map(let sections) = (try? MsgPack.decode(data)) ?? .nil
    else { return }
      ...
      case "outbound":
        for (k, v) in entries {
          guard case .bytes(let dest) = k, case .array(let arr) = v, arr.count == 2,
            let expiry = asDouble(arr[0]), case .bytes(let ticket) = arr[1]
          else { continue }
  ...
  public func getWeight(transientID: Data) -> Double {
    lock.lock()
    defer { lock.unlock() }
    guard let entry = unsafePropagationEntries[transientID] else { return 0 }
  ...
  public func getSize(transientID: Data) -> Int {
    lock.lock()
    defer { lock.unlock() }
    return unsafePropagationEntries[transientID]?.msgSize ?? 0
  }
...
private final class DeliveryAnnounceHandler: AnnounceHandler {
  let aspectFilter: String? = appName + ".delivery"
