...
enum SharedStateInventory {
  ...
  struct Entry {
    ...
    init(_ name: String, _ note: String) {
      self.name = name
      self.note = note
    }
  }
  ...
  static let router: [Entry] = [
    ...
    Entry(
      "pendingOutbound",
      "the outbound delivery queue; mutated under the lock on the send, process and cancel paths (bugs/056)"
    ),
  ]
  ...
  static func declarationLines(_ lines: [String], property name: String) -> [Int] {
    lines.indices.filter {
      stripComment(lines[$0]).range(
        of:
          #"^\s*(public |internal |private |fileprivate )?(private\(set\) |internal\(set\) )?var \#(spellings(name))\b"#,
        options: .regularExpression) != nil
    }.map { $0 + 1 }
  }
...
final class SharedStateInventoryTests: XCTestCase {
  ...
  private typealias Owner = (
    label: String, file: String, lock: String, entries: [SharedStateInventory.Entry]
  )
  ...
  func testEveryInventoriedPropertyIsAccessedUnderItsLock() throws {
    for o in owners {
      ...
      for e in o.entries {
        ...
        let sites = SharedStateInventory.lockedAccessLines(
          lines, property: e.name,
          lockName: o.lock)
        XCTAssertFalse(
          sites.isEmpty,
          """
  ...
  func testTheRouterMakesNoUnsynchronizedPeerWrites() {
    XCTAssertTrue(
      SharedStateInventory.peerCrossObjectWrites.isEmpty,
      """
  ...
  func testTheInventoriesAreDisjointAndDuplicateFree() {
    ...
    XCTAssertTrue(
      routerNames.intersection(peerNames).isEmpty,
      """
    ...
    XCTAssertTrue(
      crossNames.isSubset(of: peerNames),
      """
    ...
    XCTAssertEqual(
      SharedStateInventory.router.count, routerNames.count, "duplicate in router inventory")
    XCTAssertEqual(SharedStateInventory.peer.count, peerNames.count, "duplicate in peer inventory")
    XCTAssertEqual(
      SharedStateInventory.peerCrossObjectWrites.count, crossNames.count,
  ...
  func testLockDetectionHandlesTheThreePatternsThatFooledTheSurvey() {
    ...
    XCTAssertTrue(
      under(8, 8), "an unlock inside a returning branch must not release the fall-through path")
