...
final class LXMessageTests: XCTestCase {
  ...
  private func makeDestination(identity: Identity, aspects: [String] = ["delivery"]) throws
    -> Destination
  {
    try Destination(
      identity: identity, direction: .in, kind: .single, appName: "lxmf", aspects: aspects)
  }
  ...
  func testPackUnpackRoundTrip() throws {
    ...
    let src = try Destination(
      identity: srcIdentity, direction: .in, kind: .single, appName: "lxmf", aspects: ["delivery"])
    let dst = try Destination(
      identity: dstIdentity, direction: .in, kind: .single, appName: "lxmf", aspects: ["delivery"])
    ...
    XCTAssertGreaterThan(
      packed.count,
      LXMessage.destinationLength + LXMessage.destinationLength + LXMessage.signatureLength)
  ...
  func testOpportunisticFallsBackToDirectForLargeContent() throws {
    ...
    let msg = LXMessage(
      destination: dst, source: src, content: bigContent, desiredMethod: .opportunistic)
    try msg.pack()
