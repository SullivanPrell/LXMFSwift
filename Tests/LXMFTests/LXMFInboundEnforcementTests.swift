...
final class LXMFInboundEnforcementTests: XCTestCase {
  ...
  private func makePair() throws -> (
    router: LXMRouter, src: Destination, dst: Destination,
    ...
    let src = try Destination(
      identity: srcId, direction: .in, kind: .single,
      appName: "lxmf", aspects: ["delivery"])
    let dst = try Destination(
      identity: dstId, direction: .in, kind: .single,
      appName: "lxmf", aspects: ["delivery"])
  ...
  func testDuplicateMessageDeliveredOnce() throws {
    ...
    XCTAssertFalse(
      router.finalizeInboundDelivery(msg), "second delivery of same hash must be suppressed")
  ...
  func testTicketIngestRemembersOutboundTicket() throws {
    ...
    XCTAssertTrue(router.finalizeInboundDelivery(msg))
    XCTAssertEqual(
      router.getOutboundTicket(destinationHash: src.hash), ticket,
  ...
  func testUnsignedTicketIsNotIngested() throws {
    ...
    msg.fields[Int(Field.ticket.rawValue)] =
      [Date().timeIntervalSince1970 + 1_000_000, ticket] as [Any]

    XCTAssertTrue(router.finalizeInboundDelivery(msg))
    XCTAssertNil(
      router.getOutboundTicket(destinationHash: src.hash),
  ...
  func testInvalidStampDroppedWhenEnforced() throws {
    ...
    router.onMessageReceived = { _ in delivered = true }
    XCTAssertFalse(
      router.finalizeInboundDelivery(msg), "invalid stamp under enforcement must be dropped")
    XCTAssertFalse(delivered)
  }
  ...
  func testInvalidStampAllowedWhenNotEnforced() throws {
    ...
    router.onMessageReceived = { _ in delivered = true }
    XCTAssertTrue(
      router.finalizeInboundDelivery(msg), "invalid stamp must pass when enforcement is off")
