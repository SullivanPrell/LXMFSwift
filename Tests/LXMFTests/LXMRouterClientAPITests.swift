...
final class LXMRouterClientAPITests: XCTestCase {
  ...
  private func makeIdentityAndRegister(
    router: LXMRouter, transport: Transport,
    displayName: String? = nil,
    stampCost: Int? = nil
  ) throws -> (Identity, Destination) {
    let identity = Identity()
    let dest = try router.register(
      identity: identity,
      transport: transport,
  ...
  func testGetAnnounceAppDataNilForUnknown() throws {
    ...
    XCTAssertNil(
      router.getAnnounceAppData(destinationHash: unknown),
  ...
  func testGetAnnounceAppDataNonNilForRegistered() throws {
    ...
    XCTAssertNotNil(
      router.getAnnounceAppData(destinationHash: dest.hash),
  ...
  func testGetAnnounceAppDataNilFields() throws {
    ...
    guard let appData = router.getAnnounceAppData(destinationHash: dest.hash) else {
      XCTFail("Expected non-nil appData")
      return
    }
    guard case .array(let items) = (try? MsgPack.decode(appData)),
      items.count == 3
    else {
      XCTFail("Expected 3-element array from msgpack")
      return
    }
    ...
    guard case .array(let funcs) = items[2], funcs.count == 1 else {
      XCTFail("supported_functionality should be a 1-element array")
      return
    }
    switch funcs[0] {
  ...
  func testGetAnnounceAppDataDisplayName() throws {
    let (router, transport) = makeRouter()
    let (_, dest) = try makeIdentityAndRegister(
    ...
    guard case .bytes(let nameBytes) = items[0] else {
      XCTFail("display_name should be bytes, got \(items[0])")
      return
    }
  ...
  func testAnnounceUnknownDestinationIsNoop() {
    ...
    XCTAssertNoThrow(
      try router.announce(destinationHash: unknown),
  ...
  func testDeliveryLinkAvailableFalseWhenNoLink() throws {
    ...
    let (_, dest) = try makeIdentityAndRegister(router: router, transport: transport)
    XCTAssertFalse(
      router.deliveryLinkAvailable(destinationHash: dest.hash),
  ...
  func testGetOutboundPropagationCostNilWhenNoPNSet() {
    let (router, _) = makeRouter()
    XCTAssertNil(
      router.getOutboundPropagationCost(),
