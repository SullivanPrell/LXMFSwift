...
final class PropagationAnnouncePeeringTests: XCTestCase {
  ...
  func testAnAnnounceFromAPropagationNodeCreatesAPeer() throws {
    ...
    XCTAssertNotNil(
      net.node.peers[net.remotePropagationHash],
      """
  ...
  func testAPlainClientDoesNotPeerWithTheNodesItHears() throws {
    ...
    LXMFPropagationAnnounceHandler(router: client)
      .receivedAnnounce(
        destinationHash: net.remotePropagationHash,
  ...
  func testAStaticPeerAlreadyHeardFromIgnoresAPathResponse() throws {
    ...
    net.node.setStaticPeers([net.remotePropagationHash])
    try net.announceRemote()
    let heard = try XCTUnwrap(
      net.node.peers[net.remotePropagationHash]?.lastHeard,
    ...
    XCTAssertNotEqual(
      net.node.peers[net.remotePropagationHash]?.peeringCost, 3,
      """
  ...
  private struct Network {
    ...
    func announceRemote() throws {
      ...
      _ = try remoteTransport.announce(
        destination: dest,
    ...
    func deliverAnnounceDirectly(
      ...
      handler.receivedAnnounce(
        destinationHash: remotePropagationHash,
