...
final class ClientStatePersistenceTests: XCTestCase {
  ...
  func testOutboundStampCostsPersistAcrossRestart() {
    ...
    b.storagePath = dir
    XCTAssertEqual(
      b.getOutboundStampCost(destinationHash: dest), 12,
  ...
  func testOutboundStampCostFileIsWritten() {
    ...
    XCTAssertTrue(
      FileManager.default.fileExists(atPath: dir + "/outbound_stamp_costs"),
  ...
  func testLocallyDeliveredTransientIDsPersistAcrossRestart() {
    ...
    let a = makeRouter()
    a.storagePath = dir
    // single-threaded test; direct mutation
    a.locallyDeliveredTransientIDs[tid] = Date().timeIntervalSince1970
    a.saveLocallyDeliveredTransientIDs()

    let b = makeRouter()
    b.storagePath = dir
    XCTAssertTrue(
      b.hasMessage(transientID: tid),
  ...
  func testOutboundTicketPersistsAcrossRestart() {
    ...
    b.storagePath = dir
    XCTAssertEqual(
      b.getOutboundTicket(destinationHash: dest), ticket,
  ...
  func testInboundTicketPersistsAcrossRestart() {
    ...
    b.storagePath = dir
    XCTAssertEqual(
      b.getInboundTickets(destinationHash: dest), [generated.ticket],
