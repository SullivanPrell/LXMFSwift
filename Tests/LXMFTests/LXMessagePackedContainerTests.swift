...
final class LXMessagePackedContainerTests: XCTestCase {
  ...
  private func containerValue(for key: String, in pairs: [(MsgPack.Value, MsgPack.Value)])
    -> MsgPack.Value?
  {
    pairs.first {
      if case .string(let s) = $0.0 { return s == key }
      return false
    }?.1
  }
  ...
  func testPackedContainerIsMsgpackDict() throws {
    ...
    XCTAssertNotNil(
      containerValue(for: "lxmf_bytes", in: pairs),
  ...
  func testPackedContainerLxmfBytesMatchesPacked() throws {
    ...
    guard let lxmfBytesVal = containerValue(for: "lxmf_bytes", in: pairs),
      case .bytes(let bytes) = lxmfBytesVal
    else {
      XCTFail("lxmf_bytes must be a byte array")
      return
    }
  ...
  func testPackedContainerIncludesTransportEncrypted() throws {
    ...
    XCTAssertNotNil(
      containerValue(for: "transport_encrypted", in: pairs),
  ...
  func testWriteToDirectoryUsesHashAsFileName() throws {
    ...
    XCTAssertEqual(
      fileURL!.lastPathComponent, expectedName,
