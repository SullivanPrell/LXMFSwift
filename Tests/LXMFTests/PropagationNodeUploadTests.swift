...
final class PropagationNodeTests: XCTestCase {
  ...
  private func openClientLink() throws -> Link {
    ...
    XCTAssertTrue(
      net.waitUntil("client link up", timeout: 5) { link.status == .active },
  ...
  func testSinglePacketUploadIsIngestedAndProved() throws {
    ...
    XCTAssertTrue(
      net.waitUntil("packet proved", timeout: 5) {
  ...
  func testInvalidStampUploadIsRejectedNotProved() throws {
    ...
    XCTAssertTrue(
      net.waitUntil("link torn down", timeout: 5) { link.status != .active },
      "the reference tears the link down on an invalid-stamp upload")
    net.settle(0.2)
    XCTAssertNil(
      net.routerB.propagationEntries[transientID],
