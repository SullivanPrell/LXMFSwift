...
final class LXMessageURITests: XCTestCase {
  ...
  func testFromURIFailsWithoutTheAddressedKey() throws {
    ...
    let stranger = try Destination(
      identity: Identity(), direction: .in, kind: .single,
      appName: appName, aspects: ["delivery"])
    XCTAssertThrowsError(
      try LXMessage.fromURI(uri, destination: stranger),
  ...
  func testIngestValidURIDelivers() throws {
    ...
    let src = try Destination(
      ...
      appName: appName, aspects: ["delivery"])
    let msg = LXMessage(
      destination: delivery, source: src,
    ...
    XCTAssertEqual(
      delivered?.contentAsString, "ingest test",
  ...
  func testIngestMalformedURIThrows() {
    let router = LXMRouter(transport: Transport())
    XCTAssertThrowsError(
      try router.ingestLXMURI("not-a-uri"),
  ...
  func testIngestWrongSchemeThrows() {
    let router = LXMRouter(transport: Transport())
    XCTAssertThrowsError(
      try router.ingestLXMURI("http://example.com"),
