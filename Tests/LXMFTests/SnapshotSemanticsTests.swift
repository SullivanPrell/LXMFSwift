...
final class SnapshotSemanticsTests: XCTestCase {
  ...
  private func makeStore() throws -> (LXMRouter, [Data]) {
    ...
    for i in 0..<60 {
      ...
      router.addToMessageStore(
        lxmfData: lxmf, transientID: tid, stampValue: 5,
        ...
        for i in 0..<iterations {
          if w == 0 {
            ...
            if (w &+ i) % 2 == 0 {
              ...
              router.addToMessageStore(
                lxmfData: Data(repeating: 0x11, count: LXMessage.destinationLength) + lxmf,
  ...
  func testAMessageStoreSnapshotCopiesItsEntries() throws {
    ...
    XCTAssertEqual(
      router.propagationEntries[ids[0]]?.msgSize, original,
      """
