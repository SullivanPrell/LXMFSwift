...
final class LXMFFieldConstantsTests: XCTestCase {
  ...
  func testReactionTo() {
    XCTAssertEqual(
      ReactionField.reactionTo.rawValue, 0x00,
      "Python: REACTION_TO = 0x00")
  }

  func testReactionContent() {
    XCTAssertEqual(
      ReactionField.reactionContent.rawValue, 0x01,
  ...
  func testCommentFor() {
    XCTAssertEqual(
      CommentField.commentFor.rawValue, 0x00,
  ...
  func testContinuationOf() {
    XCTAssertEqual(
      ContinuationField.continuationOf.rawValue, 0x00,
