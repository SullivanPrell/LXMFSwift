...
final class LXMRouterStampTests: XCTestCase {
  ...
  func testGetOutboundStampCostNilWhenUnknown() {
    ...
    XCTAssertNil(
      router.getOutboundStampCost(destinationHash: unknown),
  ...
  func testSetAndGetOutboundStampCostRoundTrips() {
    ...
    XCTAssertEqual(
      router.getOutboundStampCost(destinationHash: hash), 8,
  ...
  func testHasMessageReturnsFalseWhenNotDelivered() {
    ...
    XCTAssertFalse(
      router.hasMessage(transientID: fakeID),
  ...
  func testGetOutboundProgressNilForUnknownHash() {
    ...
    XCTAssertNil(
      router.getOutboundProgress(lxmHash: unknownHash),
  ...
  func testGetOutboundProgressReturnsProgressForPendingMessage() throws {
    ...
    msg.state = .outbound
    msg.progress = 0.5
    guard let hash = msg.hash else {
      XCTFail("hash must be set after pack")
      return
    }
    router.testInjectPendingOutbound(msg)
  ...
  private func makeSrcDst() throws -> (Destination, Destination) {
    ...
    let src = try Destination(
      identity: srcID, direction: .in, kind: .single, appName: appName, aspects: ["delivery"])
    let dst = try Destination(
      identity: dstID, direction: .in, kind: .single, appName: appName, aspects: ["delivery"])
    return (src, dst)
  }
  ...
  func testDeliveryAnnounceExtractsStampCostFromAppData() throws {
    ...
    XCTAssertEqual(
      router.getOutboundStampCost(destinationHash: dstHash), 9,
  ...
  func testStampCostFromAppDataReadsIndexOne() {
    ...
    XCTAssertEqual(
      stampCostFromAppData(appData), 5,
