...
final class LXMPropagationNodeTests: XCTestCase {
  ...
  func testEnablePropagationCreatesDirectories() throws {
    ...
    XCTAssertTrue(
      fm.fileExists(atPath: tempDir + "/lxmf"),
      "lxmf/ directory must be created")
    XCTAssertTrue(
      fm.fileExists(atPath: tempDir + "/lxmf/messagestore"),
  ...
  func testEnablePropagationIndexesExistingMessages() throws {
    ...
    XCTAssertNotNil(
      router.propagationEntries[transientID],
  ...
  func testAddToMessageStoreCreatesFile() throws {
    ...
    let entry = router.addToMessageStore(
      lxmfData: lxmfBytes, transientID: tid,
  ...
  func testAddPeerDoesNotSeedTheExistingStore() throws {
    ...
    let peer = router.addPeer(destinationHash: hash)
    XCTAssertFalse(
      peer.unhandledMessages.contains(tid),
  ...
  func testFlushPeerDistributionQueueMarksPeerUnhandled() throws {
    ...
    XCTAssertTrue(
      router.peerDistributionQueue.isEmpty,
  ...
  func testHandleOfferRequestNoIdentity() throws {
    ...
    if case .int(let code) = result {
      ...
      XCTFail("Expected noIdentity error")
    }
  }
  ...
  func testHandleOfferRequestNotEnabledReturnsNoAccess() {
    ...
    if case .int(let code) = result {
      ...
      XCTFail("Expected noAccess error")
    }
  }
  ...
  func testHandleOfferRequestWantsAllNewMessages() throws {
    ...
    if case .bool(let b) = result {
      ...
      XCTFail("Expected .bool(true) when all IDs are new")
    }
  }
  ...
  func testHandleOfferRequestWantsNoneWhenAlreadyHave() throws {
    ...
    if case .bool(let b) = result {
      ...
      XCTFail("Expected .bool(false) when all IDs are already stored")
    }
  }
  ...
  func testHandleOfferRequestPartialWanted() throws {
    ...
    if case .array(let wanted) = result {
      ...
      XCTFail("Expected partial wanted array")
    }
  }
  ...
  func testHandleMessageGetRequestListAvailableMessages() throws {
    ...
    if case .array(let ids) = result {
      ...
      XCTFail("Expected array of transient IDs")
    }
  }
  ...
  func testHandleMessageGetRequestHaveListDeletesMessages() throws {
    ...
    XCTAssertNil(
      router.propagationEntries[tid], "Message must be deleted when client reports having it")
  ...
  func testSyncPeersDoesNothingWhenNotAPropagationNode() {
    ...
    router.peer(
      destinationHash: hash, timestamp: 1_000, transferLimit: 256, syncLimit: 256,
  ...
  func testGetWeightIsPriorityTimesAgeTimesSize() {
    ...
    XCTAssertEqual(
      router.getWeight(transientID: old), 400.0, accuracy: 1.0,
  ...
  func testAPrioritisedDestinationSortsFirst() {
    ...
    XCTAssertEqual(
      router.getWeight(transientID: tid), ordinary * 0.1, accuracy: 1.0,
      """
  ...
  func testGetPropagationNodeAppDataIsValidMsgpack() throws {
    ...
    guard case .array(let fields) = try? MsgPack.decode(appData),
      ...
      XCTFail("app data must be a 7-element array")
      return
    }
  ...
  func testGetPropagationNodeAppDataNotActiveWhenDisabled() throws {
    ...
    try router.register(identity: id, transport: Transport())
    let appData = router.getPropagationNodeAppData()
    guard case .array(let fields) = try? MsgPack.decode(appData) else {
      XCTFail()
      return
    }
  ...
  func testSecondRegisterDoesNotChangePropagationDestination() throws {
    ...
    XCTAssertEqual(
      router.propagationDestination?.hash, hashBefore,
  ...
  func testEnablePropagationRegistersMessageGetHandler() throws {
    ...
    XCTAssertNotNil(
      router.propagationDestination?.requestHandlers[pathHash],
  ...
  func testPropagationNodeServesMessageListNatively() throws {
    ...
    let serverIface = LoopIface(name: "S")
    let clientIface = LoopIface(name: "C")
    serverIface.paired = clientIface
    clientIface.paired = serverIface
    ...
    guard let propDest = router.propagationDestination else {
      XCTFail("propagationDestination not created")
      return
    }
  ...
  func testPropagationNodeAcceptsInboundResourceUpload() throws {
    ...
    let serverIface = LoopIface(name: "S2")
    let clientIface = LoopIface(name: "C2")
    serverIface.paired = clientIface
    clientIface.paired = serverIface
    serverTransport.register(interface: serverIface)
    clientTransport.register(interface: clientIface)
    ...
    let dstDest = try Destination(
      identity: dstId, direction: .in, kind: .single, appName: "lxmf", aspects: ["delivery"])
    ...
    let stamp = try XCTUnwrap(
      LXStamper.generateStamp(messageID: Hashes.fullHash(lxmfData), stampCost: stampCost))
