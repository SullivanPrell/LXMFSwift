...
final class ConsumerWriteParityTests: XCTestCase {
  ...
  /// because a manual sync is a one-off nudge and not a declaration that the peer is healthy—if
  /// this attempt fails too, `sync()` resumes stepping the backoff from where it was. Only a
  ...
  func testRequestingAnImmediateSyncDoesNotClearTheAccumulatedBackoff() {
    ...
    XCTAssertEqual(
      peer.syncBackoff, 60, accuracy: 0.001,
      """
  ...
  /// of a property this change made read-only. Read-only is not the reverse of write-only: the whole
