...
final class LXMStamperTests: XCTestCase {
  ...
  func testGenerateAndValidateStampCost1() throws {
    ...
    let stamp = try XCTUnwrap(
      LXStamper.generateStamp(messageID: messageID, stampCost: cost, expandRounds: expandRounds))

    XCTAssertEqual(stamp.count, 32, "stamp should be 32 bytes (SHA256 size)")
    XCTAssertTrue(
      LXStamper.stampValid(stamp: stamp, targetCost: cost, workblock: workblock),
  ...
  func testPackWithStampAddsFifthPayloadElement() throws {
    ...
    let msg = LXMessage(
      destination: dst, source: src, content: "stamp test",
    ...
    guard case .array(let parts) = try MsgPack.decode(payloadBytes) else {
      XCTFail("payload should decode as array")
      return
    }
  ...
  func testUnpackExtractsStampFromFifthElement() throws {
    ...
    let msg = LXMessage(
      destination: dst, source: src, content: "roundtrip",
      stampCost: 1, stampExpandRounds: 1)
    try msg.pack()
  ...
  func testHashNotAffectedByStamp() throws {
    ...
    let msgStamped = LXMessage(
      destination: dst, source: src, content: "same content",
      stampCost: 1, stampExpandRounds: 1)
    ...
    XCTAssertEqual(
      msgStamped.hash, msgNoStamp.hash,
  ...
  func testValidateStamp() throws {
    ...
    let msg = LXMessage(
      destination: dst, source: src, content: "validate",
      stampCost: 1, stampExpandRounds: 1)
    try msg.pack()
