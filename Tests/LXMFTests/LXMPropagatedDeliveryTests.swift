...
final class LXMPropagatedDeliveryTests: XCTestCase {
  ...
  func testPropagationPackedIsValidMsgpack() throws {
    ...
    XCTAssertEqual(
      Data(msgBytes.prefix(destLen)), msg.destinationHash,
    ...
    XCTAssertGreaterThan(
      msgBytes.count, packed.count - destLen,
  ...
  func testPropagationPackedIsEncryptedNotPlaintext() throws {
    ...
    guard case .array(let outer) = try? MsgPack.decode(pp),
      ...
      case .bytes(let lxmfData) = msgs[0]
    else {
      XCTFail()
      return
    }
    ...
    XCTAssertEqual(
      Data(decrypted.prefix(destLen)), src.hash,
  ...
  func testRouterFailsImmediatelyIfNoPropagationNode() throws {
    ...
    XCTAssertThrowsError(try router.send(msg)) { error in
      XCTAssertEqual(
        error as? LXMRouterError, .noPropagationNode,
  ...
  func testRouterSendsPropagationResourceToNode() throws {
    ...
    let msg = LXMessage(
      destination: dst, source: src, content: "store and forward", desiredMethod: .propagated)
    try msg.pack()
    ...
    XCTAssertGreaterThanOrEqual(
      inner.count, destLen, "inner bytes must be at least destHash length")
...
extension LXMPropagatedDeliveryTests {
  ...
  private func makeDeliveryPair() throws -> (
    src: Destination, dst: Destination, srcId: Identity, dstId: Identity
  ) {
    let srcId = Identity()
    let dstId = Identity()
    let src = try Destination(
  ...
  private func establishLoopbackLinks() throws -> (Transport, Transport, Link, Link) {
    ...
    let bDest = try Destination(
      identity: bId, direction: .in, kind: .single, appName: "lxmf", aspects: ["propagation"])
