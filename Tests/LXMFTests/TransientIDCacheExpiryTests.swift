...
final class TransientIDCacheExpiryTests: XCTestCase {
  ...
  func testCacheConstantsMatchPython() {
    XCTAssertEqual(
      LXMRouter.transientIDCacheExpiry, 30 * 24 * 60 * 60 * 6.0,
      "Python expires at MESSAGE_EXPIRY * 6.0")
    XCTAssertEqual(
      LXMRouter.jobTransientInterval, 60,
  ...
  func testDeliveredIDsRoundTripWithTimestamps() {
    ...
    b.storagePath = dir
    XCTAssertEqual(
      b.locallyDeliveredTransientIDs[tid] ?? 0, stamp, accuracy: 0.001,
  ...
  func testLegacyArrayFormatIsMigratedNotDiscarded() throws {
    ...
    XCTAssertTrue(
      router.hasMessage(transientID: tid),
      "a legacy timestamp-less cache must be migrated, not dropped")
    XCTAssertNotNil(
      router.locallyDeliveredTransientIDs[tid],
