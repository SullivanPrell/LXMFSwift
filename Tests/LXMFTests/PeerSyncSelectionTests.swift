...
final class PeerSyncSelectionTests: XCTestCase {
  ...
  @discardableResult
  private func addSyncablePeer(
    _ router: LXMRouter, _ index: Int,
    transferRate: Double = 0
  ) -> LXMPeer {
    router.peer(
      destinationHash: hash(index), timestamp: TimeInterval(1_000 + index),
  ...
  func testTheFastestPeersFormThePool() throws {
    ...
    XCTAssertTrue(
      selected.isSubset(of: [hash(2), hash(3)]),
      """
  ...
  func testAnUntriedPeerCanBeSelectedAgainstAnEstablishedFastOne() throws {
    ...
    XCTAssertTrue(
      selected.contains(hash(2)) || selected.contains(hash(3)),
      """
      ...
      """)
    XCTAssertTrue(
      selected.contains(hash(0)) || selected.contains(hash(1)),
