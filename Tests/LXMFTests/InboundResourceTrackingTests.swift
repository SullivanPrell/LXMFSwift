...
final class InboundResourceTrackingTests: XCTestCase {
  ...
  func testCancelUnknownResourceReturnsFalse() {
    ...
    XCTAssertFalse(
      router.cancelInbound(resourceHash: Data(repeating: 0xAB, count: 32)),
  ...
  func testJobResourceIntervalMatchesPython() {
    XCTAssertEqual(
      LXMRouter.jobResourceInterval, 2,
  ...
  func testPropagationTransferSizeStartsNil() {
    XCTAssertNil(
      makeRouter().propagationTransferSize,
  ...
  func testCancelPropagationRequestsClearsTransferSize() {
    ...
    XCTAssertNil(
      router.propagationTransferSize,
  ...
  func testMessageGetProgressPublishesStateProgressAndSize() throws {
    ...
    XCTAssertEqual(
      router.propagationTransferState, .receiving,
  ...
  private func establishLoopbackLinks(
    ...
    let aI = TrackingLoopIface(name: "A")
    let bI = TrackingLoopIface(name: "B")
    aI.paired = bI
    bI.paired = aI
