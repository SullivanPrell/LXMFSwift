...
final class PeerOutboundSyncTests: XCTestCase {
  ...
  func testTheHarnessPeersAWithBOverTheWire() throws {
    ...
    XCTAssertTrue(
      net.transportA.hasPath(to: net.bPropagationHash),
      "the announce must also leave A with a path to dial")
    XCTAssertNotNil(
      net.transportA.recall(identity: net.bPropagationHash),
  ...
  func testSyncGeneratesThePeeringKeyWhenItIsMissing() throws {
    ...
    XCTAssertTrue(
      net.waitUntil("a peering key is generated", timeout: 10) {
        peer.peeringKey != nil
      },
      """
  ...
  func testAGeneratedKeyIsAcceptedByARealReceiver() throws {
    ...
    XCTAssertNotEqual(
      answer, .int(Int64(LXMPeerError.invalidKey.rawValue)),
      """
  ...
  func testARaisedPeeringCostDiscardsAndRegeneratesTheKey() throws {
    ...
    XCTAssertTrue(
      net.waitUntil("the key is regenerated", timeout: 20) {
      ...
      """)
    XCTAssertNotEqual(
      try XCTUnwrap(peer.peeringKey).stamp, firstStamp,
  ...
  func testConcurrentPeeringKeyGenerationStartsOneGeneration() throws {
    ...
    XCTAssertEqual(
      peer.peeringKeyGenerationsStarted, 1,
      """
  ...
  func testSyncDeliversAStoredMessageToThePeerNode() throws {
    ...
    XCTAssertTrue(
      net.waitUntil("the message reaches B") {
  ...
  func testTheSyncCommitsItsStateBeforeEveryCallout() throws {
    ...
    net.routerA.syncPeers()
    XCTAssertTrue(
      net.waitUntil("first message lands") {
    ...
    net.routerA.syncPeers()
    XCTAssertTrue(
      net.waitUntil("second message lands") {
  ...
  func testTheSyncLinkIdentifiesToThePeerNode() throws {
    ...
    XCTAssertEqual(
      identifiedAs, net.identityA.hash,
      """
  ...
  func testTheOfferCarriesThePeeringKeyThenTheTransientIDs() throws {
    ...
    guard case .array(let elements)? = captured, elements.count == 2 else {
      return XCTFail(
        "the offer must be a two-element array (LXMPeer.py:385), got \(captured as Any)")
    }
  ...
  func testTheOfferIsOrderedByWeightNotReceiveTime() throws {
    ...
    let order = ids.compactMap { v -> Data? in
      guard case .bytes(let b) = v else { return nil }
      return Data(b)
    }
    XCTAssertEqual(
      order, [small, big],
  ...
  func testAThrottledResponsePostponesTheNextSync() throws {
    ...
    XCTAssertEqual(
      delay, LXMRouter.pnStampThrottle, accuracy: 3.0,
      """
  ...
  func testANoAccessResponseBreaksThePeering() throws {
    ...
    XCTAssertNil(
      net.routerA.peers[peer.destinationHash],
      """
  ...
  func testAnInvalidKeyResponseKeepsTheMessagesUnhandled() throws {
    ...
    XCTAssertFalse(
      peer.unhandledMessages.isEmpty,
      """
  ...
  func testThePeerReceivesOnlyTheMessagesItLacks() throws {
    ...
    _ = net.routerB.addToMessageStore(
      lxmfData: sharedBytes.prefix(sharedBytes.count - 32),
      transientID: shared, stampValue: 0,
      stamp: sharedBytes.suffix(32))

    net.routerA.syncPeers()
    XCTAssertTrue(
      net.waitUntil("both missing messages land") {
    ...
    for tid in [shared, onlyA1, onlyA2] {
      XCTAssertTrue(
        peer.handledMessages.contains(tid),
        """
  ...
  func testByteAccountingUsesTheUncompressedSize() throws {
    ...
    net.routerA.syncPeers()
    XCTAssertTrue(
      net.waitUntil("the message lands", timeout: 15) {
        net.routerB.peerEntryExists(tid)
      })
    ...
    XCTAssertEqual(
      peer.txBytes, expected, accuracy: 16,
      """
  ...
  func testThePersistentStrategyChainsTheNextBatch() throws {
    ...
    XCTAssertTrue(
      net.waitUntil("both messages land", timeout: 10) {
        net.routerB.peerEntryExists(first)
          && net.routerB.peerEntryExists(second)
      },
      """
  ...
  func testARemoteTeardownReturnsThePeerToIdle() throws {
    ...
    XCTAssertTrue(
      net.waitUntil("the peer returns to idle", timeout: 3) {
        peer.state == .idle
      },
      """
  ...
  func testCleanLinksReapsAStalledOutboundSyncLink() throws {
    ...
    net.routerA.syncPeers()
    XCTAssertTrue(
      net.waitUntil("the peer is mid-sync") { peer.linkForTesting != nil },
  ...
  func testThePeeringKeySurvivesARestart() throws {
    ...
    XCTAssertEqual(
      reloaded.peeringKey?.stamp, stamp,
      """
      ...
      """)
    XCTAssertEqual(
      reloaded.peeringKey?.value, value,
  ...
  func testThePeeringKeyIsPersistedInThePythonShape() throws {
    ...
    let field = pairs.first {
      guard case .string("peering_key") = $0.0 else { return false }
      return true
    }?.1
    guard case .array(let parts)? = field, parts.count == 2 else {
      return XCTFail(
        "`peering_key` must be a two-element list [stamp, value] (LXMPeer.py:113-114, :261)")
    }
  ...
  func testTheOutboundSyncMachineExposesOnlyItsEntryPoints() throws {
    ...
    let lineStart =
      source[..<syncRegion.lowerBound].lastIndex(of: "\n")
    ...
    XCTAssertEqual(
      exposed, ["sync", "generatePeeringKey", "reapStalledSyncLink"],
      """
  ...
  func testTheOutboundPathDialsAndSendsInExactlyOnePlace() throws {
    ...
    XCTAssertEqual(
      source.components(separatedBy: "Link.initiate(").count - 1, 1,
      "one dial site")
    XCTAssertEqual(
      source.components(separatedBy: "link.request(").count - 1, 1,
      "one offer-request site")
    XCTAssertEqual(
      source.components(separatedBy: "ResourceTransfer(link:").count - 1, 1,
      "one transfer site")
  }
