...
final class LXMessageTransportEncryptionTests: XCTestCase {
  ...
  private func makeMessage(desiredMethod: LXMessage.Method = .direct) throws -> LXMessage {
    ...
    let dst = try Destination(
      ...
      appName: "lxmf", aspects: ["delivery"])
    let msg = LXMessage(
      destination: dst, source: src, content: "Test",
  ...
  func testGetPropagationStampIsValid() throws {
    ...
    guard let messageID = msg.messageID else {
      XCTFail("messageID must be set after pack()")
      return
    }
    let targetCost = 1
    let stamp = msg.getPropagationStamp(targetCost: targetCost)
    guard let stamp else {
      XCTFail("getPropagationStamp returned nil")
      return
    }
    let workblock = LXStamper.stampWorkblock(
      material: messageID, expandRounds: LXStamper.pnExpandRounds)
    XCTAssertTrue(
      LXStamper.stampValid(stamp: stamp, targetCost: targetCost, workblock: workblock),
