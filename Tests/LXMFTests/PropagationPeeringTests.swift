...
final class PropagationPeeringTests: XCTestCase {
  ...
  func testAnIncomingSyncFromAPropagationNodeCreatesAPeer() throws {
    ...
    XCTAssertFalse(
      net.router.propagationEntries.isEmpty,
      "the upload did not reach the node's store — the harness, not the defect",
      file: #filePath, line: #line)

    XCTAssertNotNil(
      net.router.peers[net.remotePropagationHash],
      """
  ...
  func testAnExistingPeerIsUpdatedNotDuplicated() throws {
    ...
    router.peer(
      destinationHash: hash, timestamp: 1_000, transferLimit: 128, syncLimit: 512,
    ...
    router.peer(
      destinationHash: hash, timestamp: 2_000, transferLimit: 256, syncLimit: nil,
    ...
    XCTAssertEqual(
      first.propagationSyncLimit, 256,
  ...
  private struct Network {
    ...
    func announceRemoteAsPropagationNode(
      nodeState: Bool = true,
      peeringCost: Int = 0,
      timebase: Int64 = 1_700_000_000
    ) {
      remoteIdentity.appData = Self.propagationAnnounceAppData(
        nodeState: nodeState,
    ...
    func seedPathToRemote(hops: UInt8) {
      nodeTransport.injectPath(
        remotePropagationHash,
    ...
    static func propagationAnnounceAppData(
      nodeState: Bool,
      peeringCost: Int,
      timebase: Int64
    ) -> Data {
  ...
  private func makeNodeAndRemote() throws -> Network {
    ...
    let remotePropagationHash = try Destination(
      identity: remoteIdentity, direction: .out,
    ...
    let uploadLink = try Link.initiate(
