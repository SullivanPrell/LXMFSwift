...
final class SharedStateEncapsulationGuardTests: XCTestCase {
  ...
  private func publiclySettable(_ lines: [String], property name: String) -> String? {
    for (i, raw) in lines.enumerated() {
      ...
      guard l.contains("{") else {
        return "line \(i + 1): stored `public var` — \(l.trimmingCharacters(in: .whitespaces))"
      }
  ...
  func testNoLockGuardedPropertyIsPubliclySettable() throws {
    for o in owners {
      ...
      XCTAssertFalse(
        o.names.isEmpty, "the \(o.label) inventory is empty — this guard would pass vacuously")
  ...
  func testTheGuardActuallyFiresOnAPubliclySettableProperty() {
    ...
    XCTAssertNotNil(
      publiclySettable(stored, property: "propagationEntries"),
    ...
    XCTAssertNotNil(
      publiclySettable(computedWithSetter, property: "peers"),
    ...
    XCTAssertNil(
      publiclySettable(readOnly, property: "peers"),
      "a read-only locked accessor is the shape being asked for and must pass")

    let privateOnly = [
      "final class Thing {", "    private var unsafePeers: [Data: Int] = [:]", "}",
    ]
  ...
  private static let exemptions: [String: [String: String]] = [
    "LXMRouter.swift": [
      ...
      "propagationStampCost":
        "configuration — the ROUTER's own, distinct from LXMPeer's announced term of the same name",
    ...
    "LXMPeer.swift": [
      "msgSize":
        "field of `PropagationEntry`, a struct — reached only through the router's locked accessors, so a snapshot copies it",
  ...
  func testEveryPublicVarIsEitherInventoriedOrExempt() throws {
    for o in owners {
      ...
      for (i, raw) in lines.enumerated() {
        ...
        guard
          let m = l.range(
            of: #"^\s*public\s+(private\(set\)\s+|internal\(set\)\s+)?var\s+\w+"#,
  ...
  func testEveryComputedAccessorThatReadsGuardedStorageTakesTheLock() throws {
    for o in owners {
      ...
      for (i, raw) in lines.enumerated() {
        ...
        guard
          head.range(
            of: #"^\s*public\s+(private\(set\)\s+|internal\(set\)\s+)?var\s+\w+.*\{\s*$"#,
            options: .regularExpression) != nil
        else { continue }
        let name = head.replacingOccurrences(
          of: #"^\s*public\s+(private\(set\)\s+|internal\(set\)\s+)?var\s+"#,
        ...
        if !text.contains("\(o.lock).lock()") {
          offenders.append(
            "\(o.file):\(i + 1): `\(name)` reads guarded storage \(reads.isEmpty ? "_\(name)" : reads.map { "_\($0)" }.joined(separator: ", ")) without taking `\(o.lock)`"
          )
        }
      }
    ...
    for o in owners {
      ...
      for (i, raw) in lines.enumerated() {
        ...
        for name in o.names {
          ...
          if l.range(of: #"^\s*private\s+var\s+_\#(name)\b"#, options: .regularExpression) != nil {
            continue
          }

          let col = l.distance(from: l.startIndex, to: r.lowerBound)
          if SharedStateInventory.isUnderLock(lines, line: i + 1, column: col, lockName: o.lock) {
            continue
          }
          ...
          while k > 0 {
            if lines[k].range(
              of:
                #"^    (@discardableResult\s+)?(public |internal |private |fileprivate )?(static )?(func |init\(|deinit)"#,
              options: .regularExpression) != nil
            {
          ...
          offenders.append(
            "\(o.file):\(i + 1) in `\(enclosing.prefix(60))`: \(l.trimmingCharacters(in: .whitespaces).prefix(70))"
          )
        }
      }
  ...
  func testNoExemptionNamesAPropertyThatIsGone() throws {
    for (file, entries) in Self.exemptions {
      ...
      for (name, reason) in entries {
        XCTAssertFalse(
          SharedStateInventory.declarationLines(lines, property: name).isEmpty,
  ...
  func testTheCollidingNamesAreGuardedOnThePeerAndExemptOnTheRouter() {
    ...
    for name in colliding {
      ...
      XCTAssertTrue(
        routerExempt.contains(name), "\(name) should be an exempt LXMRouter configuration scalar")
      XCTAssertFalse(
        routerNames.contains(name),
        """
