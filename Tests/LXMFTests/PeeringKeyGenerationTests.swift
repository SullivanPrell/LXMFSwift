...
final class PeeringKeyGenerationTests: XCTestCase {
  ...
  func testTheWorkblockMatchesPythonByteForByte() {
    let wb = LXStamper.stampWorkblock(
      material: PythonPeeringVectors.peeringID,
      expandRounds: LXStamper.peeringExpandRounds)

    XCTAssertEqual(
      wb.count, PythonPeeringVectors.workblockLength,
      "25 rounds × 256 bytes (LXStamper.py:14)")
    XCTAssertEqual(
      Data(SHA256.hash(data: wb)), PythonPeeringVectors.workblockDigest,
      """
  ...
  func testAPythonGeneratedPeeringKeyValidatesHere() {
    XCTAssertTrue(
      LXStamper.validatePeeringKey(
        peeringID: PythonPeeringVectors.peeringID,
  ...
  func testTheValueOfAPythonKeyIsTheValuePythonReported() {
    ...
    XCTAssertEqual(
      LXStamper.stampValue(workblock: wb, stamp: PythonPeeringVectors.key),
  ...
  func testGenerateStampReturnsAStampAndItsValue() throws {
    ...
    let result = try XCTUnwrap(
      LXStamper.generateStamp(
        material: material, targetCost: 4,
        expandRounds: LXStamper.peeringExpandRounds))
    ...
    let wb = LXStamper.stampWorkblock(
      ...
      expandRounds: LXStamper.peeringExpandRounds)
    XCTAssertEqual(
      LXStamper.stampValue(workblock: wb, stamp: result.stamp), result.value,
  ...
  func testCancellationStopsGeneration() {
    ...
    let result = LXStamper.generateStamp(
      material: Data(repeating: 0x01, count: 32),
      targetCost: 64,
      expandRounds: LXStamper.peeringExpandRounds,
      isCancelled: { true })
