...
final class PeerSyncContextTests: XCTestCase {
  ...
  func testContextResolvesTheRemotePropagationDestination() throws {
    ...
    let ctx = try XCTUnwrap(
      net.routerA.makePeerSyncContext(for: net.peerB),
      "B's identity is recallable and it has a propagation destination")

    XCTAssertEqual(
      ctx.destination.hash, net.bPropagationHash,
      """
  ...
  func testAPeerWhoseIdentityCannotBeRecalledHasNoContext() throws {
    ...
    XCTAssertNil(
      net.routerA.makePeerSyncContext(for: stranger),
      """
  ...
  func testARouterWithNoIdentityHasNoContext() throws {
    ...
    XCTAssertNil(
      bare.makePeerSyncContext(for: peer),
  ...
  func testTheThrottleWaitIsTheRoutersOwn() throws {
    ...
    XCTAssertEqual(
      ctx.throttleWait, LXMRouter.pnStampThrottle,
  ...
  private func makeTwoRouters() throws -> TwoRouters {
    ...
    let bPropagation = try Destination(
      identity: bIdentity, direction: .out, kind: .single,
    ...
    return TwoRouters(
      routerA: routerA, peerB: peerB, transportA: transportA,
