...
final class LXMRouterPNConcurrencyTests: XCTestCase {
    ...
    for p in 0..<8 {
      router.addPeer(
        destinationHash: Data(
          [UInt8(p)] + Data(repeating: 0xEE, count: LXMessage.destinationLength - 1)))
    }
    let linkPool = (0..<8).map { _ in DummyLinkID() }
    let peerHashes = (0..<8).map { p in
    ...
    DispatchQueue.global().async {
      DispatchQueue.concurrentPerform(iterations: workers) { w in
        for i in 0..<iterations {
          ...
          case 3:
            if (w &+ i) % 9 == 0 {
              router.removePeer(destinationHash: peerHashes[(w &+ i) % peerHashes.count])
            }
          ...
          default:
            ...
            _ = router.getStampValue(transientID: tid)
          }
        }
      }
      done.fulfill()
  ...
  func testDirectPropertyReadsRaceTheRoutersOwnWrites() throws {
    ...
    let pool: [Msg] = (0..<60).map { i in
      let lxmf = destHash + Data((0..<40).map { UInt8(($0 &+ i) & 0xFF) }) + Data([UInt8(i & 0xFF)])
      return Msg(
        lxmf: lxmf, tid: Hashes.fullHash(lxmf),
        stamp: Data(
      ...
      DispatchQueue.concurrentPerform(iterations: 4) { w in
        for i in 0..<iterations {
          ...
          if w == 0 {
            ...
            let stats =
              router.clientPropagationMessagesReceived
            ...
            case 0:
              router.addToMessageStore(
                lxmfData: m.lxmf, transientID: m.tid,
